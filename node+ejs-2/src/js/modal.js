...
jQuery.extend({
    success: function(txt){
        ...
        let ele = '<div class="my-alert my-tips-'+i+'" style="height: 40px;line-height: 40px;border-radius: 4px;' +
            'background-color: #fff;position:fixed;z-index:999;left:50%;top:45%;transform:translate(-50%,-50%);' +
            'padding:0 25px;color:#333;font-size:15px;box-shadow:0px 0px 8px 0px #999;font-family:Microsoft YaHei"><i class="am-icon-check-circle"></i> '+ txt +'</div>';
    ...
    fail: function(txt){
        ...
        let ele = '<div class="my-alert my-alert-'+i+'" style="height: 40px;line-height: 40px;border-radius: 4px;' +
            'background: #fff;position:fixed;z-index:999;left:50%;top:45%;transform:translate(-50%,-50%);' +
            'padding:0 25px;color:#ff411c;font-size:15px;box-shadow:0px 0px 8px 0px #999;font-family:Microsoft YaHei"><i class="am-icon-exclamation-circle"></i> '+ txt +'</div>';
        $(document.body).append(ele);
        window.setTimeout(function(){
    ...
    loading: function (txt) {
        $('#sy-loading').remove();
        let ele = '<div id="sy-loading" style="height: 40px;line-height: 40px;border-radius: 4px;' +
